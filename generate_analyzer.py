...
class Analyzer:
    ...
    def info_general(self):
        ...
        if 'participante_id' in self.df.columns:
            ...
            if 'timestamp_inicio' in self.df.columns:
                ...
                denom = 1
        print(f'Numero de participantes: {num_participantes}')
        print(f'Numero total de respuestas: {len(self.df)}')
        print(f'Respuestas por participante: {len(self.df) / denom:.1f}')
        print()
        print('--- Distribucion demografica ---')
        print('Edad:')
    ...
    def estadisticas_molestia(self):
        ...
        if 'molestia_ref_before' in self.df.columns:
            ref_df = self.df.drop_duplicates('participante_id')
            ref_stats = ref_df['molestia_ref_before'].astype(float).describe()
            print()
            print('Molestia audio referencia (antes del cuestionario):')
            print(ref_stats)
        stats = self.df['molestia'].astype(int).describe()
        print()
        print('Molestia audios (18 respuestas por participante):')
        print(stats)
        print()
        print('--- Por tipo de ruido ---')
        ...
        print(nivel_stats)
        print()
        print('--- Por combinacion ruido x nivel ---')
    ...
    def estadisticas_fuentes(self):
        ...
        print()
        print('Frecuencia de deteccion de fuentes:')
    ...
    def generar_graficos(self):
        ...
        afectivas = ['afectiva_agradable', 'afectiva_caotico', 'afectiva_estimulante',
                     'afectiva_sinactividad', 'afectiva_calmado', 'afectiva_molesto',
                     'afectiva_conactividad', 'afectiva_monotono']
        nombres = ['Agradable', 'Caotico', 'Estimulante', 'Sin act.', 'Calmado', 'Molesto', 'Con act.', 'Monotono']
        ...
        ax4.set_title('Percepcion afectiva media')
        ax4.set_ylabel('Puntuacion (1-5)')
        ax4.set_ylim(0, 5)
        ...
        ax.set_title('Mapa de calor: Molestia media por combinacion')
        plt.colorbar(im, ax=ax, label='Molestia')
        for i in range(3):
            for j in range(3):
                ax.text(j, i, f'{molestia_heatmap.values[i, j]:.1f}',
...
with open(r'K:\Universidad\TFG\paisajesonoros\analizar_resultados.py', 'w', encoding='utf-8') as f:
    f.write(content)

print('analizar_resultados.py generado correctamente')
